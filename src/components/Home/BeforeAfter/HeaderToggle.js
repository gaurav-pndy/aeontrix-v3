...
export default function HeaderToggle({ isAfter }) {
  return (
    <div className="flex justify-center items-center gap-2 md:gap-6 px-4">
      <span
        className={` ${isAfter ? "text-white/50" : "text-white"} subtitle-text md:display-text`}
      >
        Before Aeontrix
      </span>
      ...
      {!isAfter ? (
        <motion.span className="subtitle-text md:display-text text-white/50">
          After Aeontrix
        </motion.span>
      ) : (
        <div className="inline-block subtitle-text md:display-text">
